...
export class TutorInteractorImpl implements TutorInteractor {
    ...
    async addCourse(courseData: { Name: string; Status: boolean; Description: string; Image: string; Price: number; OfferPrice: number; Duration: string; Category: string; lessons: []; TutorId: string; CreatedAt: Date; UpdatedAt: Date; }): Promise<{ course: Course | null; }> {
        try {
            const newCourse = {
    ...
    async getStudents(courseId:string): Promise<studCourse[] | []> {
        try {
            const students = await this.Repository.getStudents(courseId)
